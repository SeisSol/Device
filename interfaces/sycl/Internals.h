 ...
 */
inline nd_range<1> computeExecutionRange1D(const size_t targetWorkGroupSize, const size_t totalSize) {
  size_t factor = (totalSize + targetWorkGroupSize - 1) / targetWorkGroupSize;
  return nd_range<>{{factor * targetWorkGroupSize}, {targetWorkGroupSize}};
}

/*
 * Computes an execution range using a default size for the work group.
 */
inline nd_range<1> computeDefaultExecutionRange1D(const size_t totalSize) {
