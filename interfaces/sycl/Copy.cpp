...
void ConcreteAPI::copyTo(void* dst, const void* src, size_t size) {
  ...
  waitCheck(
      this->currentDefaultQueue().submit([&](sycl::handler& cgh) { cgh.memcpy(dst, src, size); }));
}
...
void ConcreteAPI::copyToAsync(void* dst, const void* src, size_t count, void* streamPtr) {
  auto* targetQueue =
      (streamPtr != nullptr) ? static_cast<sycl::queue*>(streamPtr) : &this->currentDefaultQueue();
  if (!this->currentQueueBuffer().exists(targetQueue))
    throw std::invalid_argument(
        getDeviceInfoAsText(getDeviceId())
